...
7. Pick the folder `C:\Repos\madiritt.github.io` and click **Select Folder**.
   ...
   Gallery, Research, Publications, Outreach, Teaching / Mentoring,
...
10. Now the round-trip test on the riskiest collection. Open
    **Publications**, click any paper's row, and in the **Title** box add one
    space at the very end of the title (nothing else). (Historical note: until
    2026-09-08 this step edited a raw BibTeX box; publications are a form now,
    saved as one YAML file per paper in `_publications/`.)
...
18. Type this and press Enter:
    ...
    purpose (the trailing space on one title in a `_publications/` file).
    Whitespace-only noise (a trailing newline) is also a pass. Note that
    `_bibliography/papers.bib` is generated by the build and gitignored, so
    it never shows here.
    A FAIL looks like: reordered keys, deleted `#` comment lines you did not
    touch, an author list rewritten into a different shape, or changes in
    files you never opened. If you see
...
7. Expected result: the popup closes and the editor loads, showing a sidebar
   with Homepage, News, Gallery, Research, Publications, Outreach,
